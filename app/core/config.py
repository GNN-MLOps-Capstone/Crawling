...
class Settings:
    ...
    read_exclude_days: int = int(os.getenv("RECO_READ_EXCLUDE_DAYS", "30"))
    read_exclude_limit: int = int(os.getenv("RECO_READ_EXCLUDE_LIMIT", "1000"))
    exposure_cooldown_seconds: int = int(os.getenv("RECO_EXPOSURE_COOLDOWN_SECONDS", "3600"))
    exposure_penalty_window_hours: int = int(os.getenv("RECO_EXPOSURE_PENALTY_WINDOW_HOURS", "24"))
    exposure_penalty_per_impression: float = float(os.getenv("RECO_EXPOSURE_PENALTY_PER_IMPRESSION", "0.25"))
    exposure_penalty_min_multiplier: float = float(os.getenv("RECO_EXPOSURE_PENALTY_MIN_MULTIPLIER", "0.2"))
